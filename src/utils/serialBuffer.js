...
class SerialBuffer extends Uint8Array {
  ...
  /** @type {number} */
  set readPos (value) {
    if (value < 0 || value > this.byteLength) throw `Invalid readPos ${value}` // eslint-disable-line
    this._readPos = value
  }
  ...
  /** @type {number} */
  set writePos (value) {
    if (value < 0 || value > this.byteLength) throw `Invalid writePos ${value}` // eslint-disable-line
  ...
  writeString (value, length) {
    if (StringUtils.isMultibyte(value) || value.length !== length) throw 'Malformed value/length' // eslint-disable-line
  ...
  readVarLengthString () {
    ...
    if (this._readPos + length > this.length) throw 'Malformed length' // eslint-disable-line
    const bytes = this.read(length)
    return BufferUtils.toAscii(bytes)
  }
